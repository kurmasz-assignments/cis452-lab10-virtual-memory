CIS 452 Lab 6:  Virtual Memory Concepts
------------------------------------------------------------------------
...
1. Specify what system you are working on (avoid WSL and other virtual machines)
...
13. How many major and minor page faults were there for this value of `COEFFICIENT`? 

14. Set `LOOP` to 2 and report the number of major and minor page faults.

15. Why does the second iteration of the loop produce so many major page faults? 

16. What does this tell you about the page replacement policy? (Hint: How many major page faults would you expect given a strict LRU replacement policy?)
...
19. On what line does the program fail? (Is it the line with `malloc`, or some other line? Why?)

* Set `COEFFICIENT` to approximately 75% of the largest value that will allow the program to complete.

* Open another window, and run the program simultaneously in each window.

20. Describe what happens, and explain why.
...
21. Does this change affect the contents of the array? If so, explain how. If not, explain why not. 

22. Describe *precisely* how does this change alters the program's memory access
    *pattern*.  What memory addresses, pages, and frames get "touched", and in what order?
    A diagram will help here.

23. Complete the table below: (If the program won't run on your computer with a coefficient of 64, either switch to EOS, or lower the coefficient)
...
24. Explain the results in the table.  Be as specific as possible. (Some of the results will have a little "noise". You don't have to account for all the noise.)
    * Can all of the difference in performance be explained by a difference in page fault rates? If not, explain what does account for the observed difference in performance. 
